...
## Ground truth & doctrine — read BEFORE anything else in this repo

1. **`_fleet-state.md`** (repo root) — machine-generated by KIREEK's sync loop
   on Kevin's PC: where the business ACTUALLY is right now (Kevin's current
   mode, live systems health, every team's real state). It supersedes every
   dated note, status log, and your own memory of previous runs. If its
   timestamp is more than ~12 hours old, the loop is down — say so plainly and
   treat everything as unverified instead of trusting stale state.
2. **`_acquisition-doctrine.md`** (repo root) — the research-backed WHY behind
   how we work: what the goal is (client acquisition), what your work cannot
   succeed without (your team's gate in §7), and the Tier 1/2/3 claim rules.
   Anything a prospect or client might see must comply — Tier 3 stats are

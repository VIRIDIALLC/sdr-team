...
- 2026-09-11 (Elly): first touch drafted into `outreach.md`, advanced to `drafted`. Nothing sent. Package track. Angle that won: channel monoculture, the phone is the only way in (contact and about pages 404, no published email, all first-hand). Dropped: the anonymous owner, collides with Guthrie. Email GUESSED, verify before send. No response-time hook used: the Yelp advertisers technique is paused under the 2026-09-10 ruling, so every draft in this batch uses the underlying-problem opener.
